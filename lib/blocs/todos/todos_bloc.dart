...
class TodosBloc extends Bloc<TodosEvent, TodosState> {
  ...
  Stream<TodosState> mapEventToState(TodosEvent event) async* {
    logMessage('TodosBloc: ${event.toString()}!');
  ...
  Stream<TodosState> _mapAddTodoToState(
    ...
    if (currentState is TodosLoaded) {
      final List<Todo> updatedTodos = List.from(currentState.todos)..add(event.todo);
  ...
  Stream<TodosState> _mapToggleAllToState(TodosState currentState) async* {
    if (currentState is TodosLoaded) {
      ...
      final List<Todo> updatedTodos = currentState.todos.map((todo) => todo.copyWith(complete: !allComplete)).toList();
      yield TodosLoaded(updatedTodos);
      _saveTodos(updatedTodos);
    }
  }
  ...
  Stream<TodosState> _mapClearCompletedToState(TodosState currentState) async* {
    if (currentState is TodosLoaded) {
      final List<Todo> updatedTodos = currentState.todos.where((todo) => !todo.complete).toList();
      yield TodosLoaded(updatedTodos);
      _saveTodos(updatedTodos);
    }
  }
